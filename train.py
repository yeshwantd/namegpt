...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.sa_heads(x) # added residual connection. Size is (B, T, C)
        x = x + self.ffwd(x) # added residual connection. Size is (B, T, C)
        return x

class BigramLanguageModel(nn.Module):

    def __init__(self, vocab_size, block_size, head_size, num_heads, embedding_dim=64):
        super().__init__()
        ...
        self.lm_head = nn.Linear(head_size, vocab_size) # (B, T, C)
...
def train():
    ...
    m = BigramLanguageModel(vocab_size, block_size, head_size, num_heads, n_embeddings)
